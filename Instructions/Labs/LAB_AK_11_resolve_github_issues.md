...
1. リポジトリの **[Issues]** タブを選択し、少し時間を取って [Issues] ページを確認します。
    ...
    10 件の開いている issue が一覧表示されます。 issue に関する次の詳細に注意してください。
...
1. 次の重大および高優先度の issue を選択し、**[割り当て]** ドロップダウンを使用して自分に割り当てます。
    ...
    通常は、優先度の高い issue から先に取り組むのが最善の方法です。 issue を自分に割り当てると、修復プロセスを進める際に進行状況を追跡するのに役立ちます。
...
1. 下方向にスクロールして、**PaymentInfo** クラスを見つけます。
    ...
    CardNumber プロパティとカード検証コード (CVV) プロパティに関するコメントに注目してください。 このコードは、自分に割り当てた、**クレジット カードのデータ保存違反を修正する** という issue に関連しています。
...
1. GitHub Copilot チャット ビューを開き、**[質問]** モードが選択されていること、**GPT-4.1** モデルが選択されていることを確認します。
    ...
    > **注**:GitHub Copilot には、さまざまなチャット モデルが用意されています。 GPT-4.1 モデルは、優れたコード分析機能を提供し、GitHub Copilot Free プランに含まれています。 別のモデルを選択すると、得られる結果が異なる場合があります。

1. クリーンなチャット セッションから開始していることを確認します。

    チャット セッションは、GitHub Copilot との対話を整理するのに役立ちます。 各セッションは独自のコンテキストを維持するため、特定のタスクや issue に集中できます。 セッション内の会話履歴により継続性が維持されるため、GitHub Copilot は以前の対話に基づいてより正確で関連性の高い応答を提供することができます。 このチャット会話では、ContosoShopEasy アプリケーション内の自分に割り当てられた 2 つのセキュリティの脆弱性を分析して解決することに重点を置きます。 GitHub Copilot の Ask モードを使用して GitHub の issue の分析を完了したら、同じ会話を使用して、GitHub Copilot の Agent モードを使用してコード変更を実装できます。 GitHub Copilot は、Ask モードからの詳細な分析を使用して、Agent モードでコード生成を通知します。これにより、修正は、特定された脆弱性および推奨される修復戦略と確実に一致します。
...
1. 少し時間を取って、GitHub Copilot の修復提案を確認します。
    ...
    SQL インジェクション リスクの管理に役立つパラメーター化クエリまたはオブジェクト リレーショナル マッピング (ORM) メソッドを使用するための推奨事項が表示されます。 入力の検証とサニタイズの手法に関する提案も表示される場合があります。 多くの場合、GitHub Copilot により、提案を実装する方法を示すコード スニペットが提供されます。
...
1. コード エディターで、**PaymentInfo** クラス内の **CardNumber** プロパティと **CVV** プロパティを選択します。
    ...
    これらのプロパティがセキュリティの脆弱性であることを示すコメントに注意してください。 完全なカード番号と CVV コードを保存すると、Payment Card Industry Data Security Standard (PCI DSS) コンプライアンス要件に違反します。
...
1. ファイルを調べて、PaymentInfo オブジェクトが処理されるかどうかを確認します。
    ...
    OrderRepository クラスには、PaymentInfo などの Order オブジェクトが保存されることに注意してください。 PaymentInfo クラスに完全なカード番号と CVV コードが保存されると、この機密データがリポジトリに保持されます。
...
1. GitHub Copilot の分析を確認します。
    ...
    GitHub Copilot によって、Order オブジェクトおよび PaymentInfo オブジェクト内のあらゆるデータがリポジトリで保持されることが説明されます。 PaymentInfo モデルが安全なデータ (トークン、最後の 4 桁) のみを保存するように修正されていれば、リポジトリにも安全なデータが自動的に保存されます。
...
GitHub Copilot のエージェント モードを使うと、複数のファイルとメソッドで複雑なセキュリティ修正を自律的に実装できます。 分析と推奨事項を提供する質問モードとは異なり、エージェント モードでは、コードを直接変更してセキュリティの改善を実装できます。 このアプローチは、複数の関連する脆弱性に一貫して対処する必要がある体系的なセキュリティ修復に効果的です。
...
1. 少し時間を取って、修復戦略を検討します。
    ...
    GitHub Copilot の Ask モードを使用して、完了した分析を使用する修復戦略を作成します。 自分に割り当てた issue に対処する順序、issue を解決するためのアプローチ、コードの脆弱性が正常に修復されたことを確認する方法を検討します。
    ...
    重大度はクレジット カードの保存の問題の方が高くなりますが、修正は SQL インジェクションの問題の方が簡単であるため、先に対処できます。 この方法により、より複雑なクレジット カードの保存違反に取り組む前に、簡単な修正でワークフローの妥当性を確認できます。
...
1. OrderRepository への影響を検討します。
    ...
    OrderRepository.cs ファイルには、PaymentInfo を含む Order オブジェクトが保存されます。 あなたは、セキュリティで保護されたデータ (下 4 桁、カードの種類) のみを格納するように PaymentInfo クラスを更新しました。 その結果、リポジトリには完全なカード番号および CVV コードではなく、セキュリティで保護されたデータが自動的に保持されます。 リポジトリを直接変更する必要はありませんが、テスト中にデータの安全性を検証する必要があります。
...
1. 完成したアプリケーションを実行して、全体的な動作を確認します。
    ...
    出力を元のアプリケーション実行の記録と比較します。 機密性がそれほど高くない情報が記録されるようになっているはずです。
...
1. 残っている問題や改善する領域を文書化します。
    ...
    さらに注意が必要なセキュリティ上の問題や、対処が必要な機能上の問題を記録しておきます。
...
1. Visual Studio Code の [ソース管理] ビューを開き、更新された各ファイルに加えられた変更を確認します。
    ...
    修復プロセス中に予期しない変更が導入されていないか確認します。 すべての変更が修復戦略と一致し、新しい脆弱性が導入されていないことを確認します。
