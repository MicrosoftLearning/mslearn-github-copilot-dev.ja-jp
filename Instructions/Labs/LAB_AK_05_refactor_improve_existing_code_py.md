...
1. Visual Studio Code のエクスプローラー ビューで、`LabFiles/05-python-refactor-improve-existing-code/AccelerateDevGHCopilot` フォルダーに移動し、次のプロジェクト構造を確認します。

    - AccelerateDevGHCopilot/library   ├── application_core   ├── console   ├── infrastructure   └── tests

1. 次のセクションでテストを実行して、初期コードが正常に実行されることを確認します。 以前のラボで慣れている場合は、必要に応じて、ターミナルで **`python console/main.py`** を使用して、**LabFiles/05-python-refactor-improve-existing-code/AccelerateDevGHCopilot/library** フォルダーからアプリケーションを実行してください。
