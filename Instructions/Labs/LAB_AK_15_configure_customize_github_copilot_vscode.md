...
この演習では、カスタムの Copilot 指示を使用するように ContosoInventory C# Web API プロジェクトを構成し、ハンドオフを通じて共同作業を行い、開発タスクをエンドツーエンドで完了するカスタム エージェントを作成します。
...
- C# 開発キットおよび GitHub Copilot Chat の拡張機能を含む Visual Studio Code (バージョン 1.106 以降)。
...
1. すべての AI 生成のコードが確立された規則に従えるように、クライアントのコーディング標準を Copilot の動作に埋め込むカスタム指示ファイルを作成します。
...
Copilot カスタマイズ ファイルを追加する前に、既存のアプリケーション機能について理解しておく必要があります。
...
1. 少し時間を取って、プロジェクト構造を確認してください。
    ...
    Visual Studio Code のエクスプローラー ビューでプロジェクト フォルダーを展開します。 次のようなフォルダー構造が表示されます。
...
1. **ContosoInventory.Server/Services/CategoryService.cs** ファイルを開き、サービスの実装を確認します。
    ...
    使用されている次のパターンに注目します。すべてのデータベース操作の async/await、DTO マッピング、構造化ログ、入力検証、try-catch ブロックを使用したエラー処理。 演習の後半で製品機能を作成するときに、Copilot エージェントで、これらのパターンが参照されます。
...
1. 管理者のデモ用資格情報を使用してサインインします。

    メールに `mateo@contoso.com`、パスワードに `Password123!` を入力し、**[サインイン]** を選択します。

1. ホーム ページにカテゴリの統計情報が表示されることを確認します。

    ウェルカム メッセージと統計情報が表示され、カテゴリが合計 8 つあり、7 つがアクティブ、1 つが非アクティブです。

1. **[カテゴリ]** ページに移動し、8 つのカテゴリがすべて一覧表示されていることを確認します。
...
1. ログアウトして、閲覧者の資格情報を使用してサインインします。
    ...
    メールに `megan@contoso.com`、パスワードに `Password123!` を入力します。

1. **[カテゴリ]** ページに移動し、管理者専用のアクション ボタン ([カテゴリの追加]、[編集]、[非アクティブ化/アクティブ化]、[削除]) が表示されていないことを確認します。

    閲覧者として、Megan はカテゴリ一覧を表示できますが、データを変更することはできません。
...
1. GitHub Copilot が Visual Studio Code でアクティブであることを確認します。
    ...
    VS Code ウィンドウの下部にあるステータス バーで GitHub Copilot アイコンを探します。 アイコンが表示されていて、警告はありません。 Copilot がアクティブでない場合は、アクティビティ バーの [アカウント] アイコンを使用して GitHub アカウントにサインインします。

1. アクティビティ バーの [チャット] アイコンを選択するか、**Ctrl + Alt + I** キーを押して、Copilot Chat ビューを開きます。

    Copilot Chat が正常に開かれたことを確認します。 この演習では、Copilot Chat を幅広く使用します。

## コーディング標準を適用するためのリポジトリ レベルのカスタム指示を作成する

このタスクでは、プロジェクトの常時有効なガイドラインを Copilot に提供する `.github/copilot-instructions.md` ファイルを作成します。 これらの手順は、ワークスペース内のすべての Copilot Chat 要求に自動的に含まれます。

そのためには、以下の手順を実行してください。

1. Visual Studio Code のエクスプローラー ビューで、プロジェクトのルート フォルダーを右クリックし、**[新しいフォルダー]** を選択します。

1. フォルダーに **.github** という名前を付けます。

    > **注**:フォルダー名はピリオドで始まる必要があります。 これは、GitHub で構成ファイルに使用される規則であり、ここが Copilot 指示ファイルの標準的な場所です。
...
1. Copilot により、カスタム指示が取得されていることを確認します。

    Copilot Chat ビューを開き、次のプロンプトを入力します。

    ```plaintext
    What naming convention should I use for private fields in this project?
    ```

    Copilot がカスタム指示を参照し、プライベート フィールドにアンダースコアをプレフィックスとして付けるというガイダンスで応答します。 Copilot Chat の応答で、`copilot-instructions.md` を引用する **[参照]** セクションを探して、手順が適用されていることを確認します。

    > **注**:参照されている手順が表示されない場合は、`chat.includeApplyingInstructions` の設定が VS Code の設定で有効になっていることを確認します。 この設定は既定でオンになっています。

1. 指示の別の側面をテストします。

    Copilot Chat で、次のプロンプトを入力します。

    ```plaintext
    How should I structure data access in this project?
    ```

    Copilot は、カスタム指示で指定されているリポジトリ パターンに言及するはずです。
...
1. パス固有の指示がアクティブであることを確認します。

    エディターで **ContosoInventory.Server/Controllers/CategoriesController.cs** ファイルを開き、Copilot Chat ビューを開きます。 チャット ビューで右クリックし、**[Diagnostics]** を選択して、読み込まれた指示ファイルを調べます。

    `copilot-instructions.md` (リポジトリ全体) と `controllers.instructions.md` (コントローラーのパス固有) の両方がアクティブな指示として一覧表示されます。

    > **注**:Copilot で `applyTo` パターンに一致するファイルが処理される場合、パス固有の指示がリポジトリ全体の指示と自動的にマージされます。 `description` フィールドはセマンティック マッチングにも使用されるため、一致するファイルが開かれていない場合でも、コントローラーまたはサービスについて質問すると、Copilot でこれらの指示が含まれる場合があります。
...
1. プロンプト ファイルがスラッシュ コマンドとして登録されていることを確認します。

    Copilot Chat ビューを開き、チャット入力に「`/`」と入力します。 使用可能なスラッシュ コマンドの一覧に `generate-api-docs` が表示されます。

    > **注**:プロンプト ファイルがスラッシュ コマンド リストに表示されない場合は、`chat.promptFiles` 設定が VS Code 設定で有効になっていることを確認します。 これを確認するためには、[設定] (**Ctrl + ,**) を開き、「プロンプト ファイル」を検索します。

1. プロンプト ファイルをテストします。

    エディターで **ContosoInventory.Server/Controllers/CategoriesController.cs** ファイルを開き、Copilot Chat で次のように入力します。

    ```plaintext
    /generate-api-docs
    ```

    Copilot で CategoriesController が分析され、6 つのエンドポイントすべて (GET all、GET by ID、POST、PUT、DELETE、toggle-active) の書式設定された API ドキュメントが作成されます。 これは、既存のコントローラーに、異なる HTTP メソッド、承認レベル、応答の種類を持つ複数のエンドポイントがあるため、プロンプト ファイルによりチーム全体で共通タスクが標準化され、意味のある出力が表示される方法を示しています。
...
1. Copilot Chat の [エージェント] ドロップダウンに [Planner] エージェントが表示されることを確認します。

    Copilot Chat ビューで、[エージェント] ドロップダウンを開きます (通常、"Copilot" または現在のエージェントの名前が表示されます)。 使用可能なエージェントの一覧に **planner** が表示されます。

    > **ヒント**:Copilot Chat に「`/agents`」と入力して、使用可能なすべてのエージェントをすばやく表示し、切り替えることもできます。

1. **planner** エージェントを選択します。

    チャットの入力プレースホルダー テキストが "機能要件を分析し、コードを記述せずに実装計画を生成します" に変わることに注目します。これは、エージェントの frontmatter で定義した説明です。

1. 次のプロンプトを入力して、Planner エージェントをテストします。
...
1. [エージェント] ドロップダウンを開き、**[Copilot]** を選択して、既定の Copilot エージェントに戻ります。
...
1. エージェントが [エージェント] ドロップダウンに表示されていることを確認します。

    Copilot Chat の [エージェント] ドロップダウンを開きます。 これで、**planner** と **implementer** の両方が一覧表示されているはずです。

    Planner エージェントに [実装の開始] ハンドオフ ボタンが含まれていることに注目します。 このハンドオフは、選択された時に Implementer エージェントに切り替え、会話のコンテキストを引き継ぐ構成になっています。
...
1. [エージェント] ドロップダウンに 3 つのエージェントがすべて表示されていることを確認します。
    ...
    Copilot Chat の [エージェント] ドロップダウンを開きます。 **planner**、**implementer**、**reviewer** が組み込みエージェントと共に一覧表示されます。
...
1. Copilot Chat ビューを開き、[エージェント] ドロップダウンから **planner** エージェントを選択します。
...
1. Planner エージェントによって作成された計画を確認します。
...
1. 計画に問題がなければ、**[実装の開始]** ハンドオフボタンを選択します。
    ...
    このボタンは、Planner エージェントの応答の最後に表示されます。 選択すると、Copilot Chat は次のように動作します。
...
1. 入力済みのプロンプトを確認し、**[送信]** を選択して (または **Enter**キーを押して) 送信します。
    ...
    Implementer エージェントは、Planner の計画に基づいてコードの記述を開始します。 モデル、DTO、サービス インターフェイス、サービス実装、コントローラーを作成します。 ファイルが作成され、カスタム指示ファイルのコーディング標準に従っていることを確認します。
...
- **リポジトリ レベルのカスタム指示** (`.github/copilot-instructions.md`) を作成し、チームのコーディング標準 (名前付け規則、アーキテクチャ パターン、エラー処理、ドキュメント要件) をすべての Copilot Chat のやり取りに埋め込みました。
...
- **API ドキュメントの生成をスラッシュ コマンドとして標準化する再利用可能なプロンプト ファイル** (`.github/prompts/generate-api-docs.prompt.md`) を作成し、既存の CategoriesController に対してテストしました。
- **3 つのカスタム エージェント** (Planner、Implementer、Reviewer) を定義し、各開発ロールに合わせた指示、ツールのアクセス許可、動作ガイドラインを定義しました。
- エージェント間で**ハンドオフを構成し**、各移行時に開発者の監視を伴う、構造化された計画 → 実装 → レビュー ワークフローを作成しました。
