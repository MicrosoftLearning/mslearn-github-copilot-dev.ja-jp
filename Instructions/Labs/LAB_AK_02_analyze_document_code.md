...
あなたの同僚は、図書館アプリケーションの初期バージョンを開発し、トレーニング リポジトリで使用できるようにしました。 リポジトリをクローンし、Visual Studio Code で図書館ソリューションを開く必要があります。
...
1. Visual Studio Code のエクスプローラー ビューで、`LabFiles\02-analyze-document-code\AccelerateDevGHCopilot` フォルダーに移動し、次のソリューション構造を確認します。
...
1. ソリューションが正常にビルドされていることを確認します。
    ...
    たとえば、エクスプローラー ビューで **LabFiles\02-analyze-document-code\AccelerateDevGHCopilot\src\Library.Console\Library.Console.csproj** を右クリックし、**[ビルド]** を選択します。
...
1. チャット モードを **[質問]** に設定し、**[Auto]** モデルを選択します。
    ...
    > **注**: プランで許可されている場合は別のモデルを使用できますが、応答は、この演習で示されているものとは異なる可能性があります。 無料プランのユーザーは、毎月の **[AI クレジット]** の数が限られているため、各プロンプトはクォータにカウントされます。
...
1. ドラッグ アンド ドロップ操作を使用して、**JsonData.cs**、**JsonLoanRepository.cs**、**JsonPatronRepository.cs** の各ファイルを**エクスプローラー** ビューからチャット コンテキストに追加します。

    GitHub Copilot はチャット コンテキストを使用して、プロンプトに関連するコード ファイルを理解します。 ファイルをチャット コンテキストに追加するには、ドラッグ アンド ドロップ操作を使用して **[ファイルをチャットに追加する]** を右クリックして選択するか、チャット ビューの **[コンテキストの追加]** ボタンを使用します。
...
1. **エクスプローラー** ビューで **ライブラリ コンソール**を開いていることを確認します。

1. アプリケーションを実行するには、**src\Library.Console\Library.Console.csproj** を右クリックして、**[デバッグ]** を選択し、**[新しいインスタンスを開始]** を選択します。
...
この演習では、GitHub Copilot を使用してコードベースを分析し、文書化する方法を学びました。 質問モードのチャット ビューで、チャット参加者、チャット変数、スラッシュ コマンドを使用してプロジェクト構造、キー クラス、データ アクセス クラスの説明を生成しました。 チャットのコンテキストにドラッグ アンド ドロップでファイルを追加することで、応答の関連性を向上させました。 また、その動作を理解するためにアプリケーションを実行し、GitHub Copilot を使用して README.md ファイルのコンテンツを生成しました。
