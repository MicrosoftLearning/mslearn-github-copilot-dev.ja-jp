...
似た機能や関連する機能を含むコードベースを開発または拡張するとき、コード ロジックが重複することがよくあります。 それは意図的なものではなく、新機能のプロトタイプを作成するためにコードを再利用しているという単純な理由の可能性もあります。 時間が経ち、重複したロジックが周囲のコードに合わせて進化すると、問題がより複雑になる可能性があります。 ある場所で変数名、関数名、コード構造を変更し、他の場所では変更しないと、重複したロジックがわからなくなる可能性があります。 厳しいスケジュール、稚拙なドキュメント、適切なコード レビューの欠如により、問題が悪化する可能性があります。 最終的に、重複したロジックにより、コードの理解、保守、デバッグ、テストが困難になります。
...
1. Visual Studio Code のエクスプローラー ビューで、`LabFiles\07-consolidate-duplicate-code` フォルダーに移動し、次のプロジェクト構造を確認します。
...
1. 主要な処理クラスを調べます。
    ...
    **OrderProcessor.cs** と **ReturnProcessor.cs** を並べて開きます。 これらのクラスは、それぞれ顧客の注文と返品を処理するための主要なビジネス ロジックを表します。
    ...
    2 つのクラスは、メソッドのシグネチャと処理パターンが似ていることに注意してください。 この類似性は最も明白な種類の重複ですが、コードベース全体に他にももっと微妙な重複が存在します。
...
1. アプリケーションを実行し、コンソールの出力を確認します。
    ...
    エクスプローラー ビューからアプリケーションを実行するには、**ECommerceOrdersAndReturns** を右クリックし、**[デバッグ]**、**[新しいインスタンスの開始]** の順に選択します。
...
1. OrderProcessor と ReturnProcessor ファイルをチャットのコンテキストに追加します。
    ...
    ドラッグ アンド ドロップ操作を使用して、**OrderProcessor.cs** ファイルと **ReturnProcessor.cs** ファイルをエクスプローラーからチャット コンテキストに追加します。

    チャット コンテキストにファイルを追加すると、プロンプトを分析するときにそのファイルをコンテキストに含めるよう GitHub Copilot に指示されます。

    エクスプローラーではなく既定のフォルダー ビューを使用している場合は、ファイルを右クリックして **[ファイルをチャットに追加する]** を選択します。 コード エディターでファイルを開いて、コンテキストを確立することもできます。
...
1. 少し時間を取って、GitHub Copilot の応答を確認してください。
    ...
    GitHub Copilot によって、注文確認と返品確認の処理に関する重複ロジックが特定されている必要があります。 応答には、メールの準備と送信に関するテンプレート化されたアプローチが含まれます。 GitHub Copilot では、ヘルパー メソッドとコンソール出力に関連する重複パターンが特定される可能性もあります。
...
GitHub Copilot の質問モードは、単純なコピー貼り付けシナリオに留まらない微妙な重複を特定する場合に適しています。 似た論理パターン、実装方法が異なる同等のビジネス ルール、複数のファイルにまたがるアーキテクチャの重複を認識できます。

> **注**:このタスクの間に生成された分析を使って、次のセクションで実装するリファクタリング戦略が通知されます。

### GitHub Copilot Chat (エージェント モード) を使って重複ロジックを統合する

GitHub Copilot のエージェント モードを使うと、複数のファイルとアーキテクチャ レイヤーにまたがるやや複雑な複数ステップのリファクタリング タスクを割り当てることができます。 エージェントは、新しいファイルを自律的に作成したり、既存のコードを変更したり、進行状況をユーザーに常に知らせながら包括的なリファクタリング戦略を実装したりできます。
...
1. GitHub Copilot チャット ビューをエージェント モードに切り替えます。
    ...
    エージェント モードを使うと、GitHub Copilot はコードベースを自律的に変更できます。 エージェント モードは、やや複雑な複数ステップのリファクタリング タスクに適しています。
...
1. エージェントの進行状況をチャット ビューで監視します。
    ...
    エージェントが行っているタスクの処理を支援するため、続行するためのアクセス許可を付与し、必要に応じて追加のコンテキストを指定します。 たとえば、エージェントからアプリケーションのビルドまたは実行のアクセス許可を求められたら、**[続行]** を選びます

    このタスク中に、エージェントによって以下のステップが実行されます。

    - Services フォルダーに新しい **ValidationService.cs** ファイルを作成します
    - 再利用可能なメソッドに検証ロジックを抽出します
    - 新しいサービスを使うように両方のプロセッサ クラスを更新する
