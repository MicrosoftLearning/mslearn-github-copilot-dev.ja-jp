...
1. 利害関係者の要件と spec.md を使用して plan.md ファイルを生成します。
...
1. 規約ワークフローによって、**templates** フォルダー内のファイルが更新されていることに注意してください。

    規約ワークフローは、関連する GitHub Spec Kit ファイルを更新します。 この場合は、テンプレート フォルダー内の 1 つ以上のファイル (spec-template.md、plan-template.md、tasks-template.md) を意味します。 更新は、constitution.md ファイルに定義されている原則を反映しているはずです。 関連するファイルを更新すると、すべてのプロジェクト ドキュメントの一貫性を確保するのに役立ちます。

1. すべてのファイル更新を保存するには、チャット ビューで **[保持]** ボタンを選択します。

    エディターで **[保持]** ボタン オプションを選択して、個々のファイルへの変更、またはファイル内の個々の変更を受け入れることもできます。

## 利害関係者のドキュメントを使用して規約を更新する

/speckit.constitution ワークフローでは、テキスト入力、ファイル入力、コードベースを使用して、constitution.md ファイルに含めるポリシー、標準、要件、ガイドラインを抽出します。 詳細な入力を提供すると、より正確で包括的な規約を生成するのに役立ちます。
...
1. 利害関係者のドキュメントと GitHub Spec Kit コマンドの間の関係を検討します。
    ...
    利害関係者の各ドキュメントは、仕様駆動型開発プロセスを導くのに役立つ情報を提供します。
...
1. GitHub Copilot の応答を監視します。
    ...
    GitHub Copilot でプロジェクトの要件が分析され、規約のドキュメントが作成されるまでに 1 から 2 分かかる場合があります。

1. GitHub Copilot が constitution.md ファイルの更新を完了したら、少し時間を取って、推奨される編集内容を確認します。

    規約ワークフローは、利害関係者のドキュメントで指定された基になる原則を抽出し、それらを使用して詳細を規約に追加します。
...
## 利害関係者の要件を使用して spec.md ファイルを生成する
...
1. GitHub Copilot の応答を監視し、必要に応じて支援します。
    ...
    spec.md ファイルの作成と検証には 4 から 6 分かかる場合があります。 spec.md ファイルが作成される前にワークフロー プロセスが停止した場合は、GitHub Copilot の **retry** コマンドを使用してワークフローを再起動できます。
...
## 利害関係者の要件と spec.md を使用して plan.md ファイルを生成する
...
1. 少し時間を取って、**research.md**、**plan.md**、**quickstart.md**、**data-model.md** ファイルをレビューします。
    ...
    > **重要**:このラボでは、.NET 8 以降の使用がサポートされています。 ただし、利害関係者のドキュメントでは、.NET 10 がターゲット フレームワークとして指定されています。 環境で .NET 8 または .NET 9 を使用している場合は、それに応じて plan.md ファイルと quickstart.md ファイルを更新する必要があります。
...
1. **tasks.md** ファイルを開き、**実装戦略**セクションを見つけて、推奨される MVP 戦略を確認します。
    ...
    MVP 戦略は、可能な限り迅速に作業機能を配布することを目的としています。 最初のユーザー ストーリー (US1) を構築する前に、まず "セットアップ" (初期化) フェーズと "基盤" (ブロック) フェーズを完成することに重点を置く必要があります。
...
1. チャット ビューで、MVP 戦略の実装ワークフローを開始するコマンドを入力します。
    ...
    > **注**:段階的なアプローチに従う場合は、まず "セットアップ" および "基盤" フェーズのタスクを実装し、その後に "ユーザー ストーリー 1" (US1) のタスクを実装できます。
...
1. ブラウザーを開き、フロントエンド アプリケーションの [サブスクリプション] ページ (たとえば、`http://localhost:5239/subscriptions`) に移動します。
