...
1. Visual Studio Code のエクスプローラー ビューで、`LabFiles/04-python-develop-unit-tests-pytest/AccelerateDevGHCopilot` フォルダーに移動し、次のプロジェクト構造を確認します。
...
1. チャット ビューで、**[モードの設定]** ボタンを選んでから、**[編集]** を選びます。
    ...
    編集モードを使って、選んだファイルを更新します。 応答は、コード エディターにコード提案として表示されます。
...
## まとめ
<!-- HERE -->
この演習では、GitHub Copilot を使用して Python アプリケーションでの単体テストの開発を高速化し、Pytest を使用して実行する方法を学びました。 GitHub Copilot のチャット ビューを質問モード、エージェント モード、編集モードで使用しました。 質問モードを使って既存の単体テストのアプローチを調べ、エージェント モードを使ってプロジェクト フォルダーと新しいテスト クラスを作成し、編集モードを使って単体テストを作成しました。 また、GitHub Copilot のコード入力候補機能を使って単体テストを作成しました。
